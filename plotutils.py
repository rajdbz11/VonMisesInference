...
def DisplayCircularMoments1(local_inputs, beliefs, K):
	...
	ax[0].plot([l_cos2x.min(), l_cos2x.max()],[l_cos2x.min(), l_cos2x.max()],color='cyan',linewidth=1,alpha=0.5)
	...
	ax[1].plot([l_sin2x.min(), l_sin2x.max()],[l_sin2x.min(), l_sin2x.max()],color='cyan',linewidth=1,alpha=0.5)
	ax[1].set_title(r'$s_1(b)$ vs. $s_1(\phi)$', fontsize=14)
	ax[2].plot(l_cos4x.flatten(), b_cos4x[...,1:].flatten(),'b.',markersize=1)
	ax[2].plot([l_cos4x.min(), l_cos4x.max()],[l_cos4x.min(), l_cos4x.max()],color='cyan',linewidth=1,alpha=0.5)
